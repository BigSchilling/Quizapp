...
import { Dropdown, Button, InputGroup, Form, Card, Container } from "react-bootstrap";
...
const StartPage = () => {
    ...
    const sendMessage = () => {
        ...
        if (socket) {
            // Sende die Nachricht an den Server
            socket.emit('sendMessage', { userID: userID, inputMessage: inputMessage });
    ...
    const handleKeyDown = (e) => {
        ...
        if (!isInputFocused && e.keyCode === 32) { // Überprüfung, ob die gedrückte Taste die Leertaste ist und das Button-Element vorhanden ist
    ...
    const handleClickOutside = () => { // Textfeld hat den Fokus verloren
    ...
    let formattedInput = input.replace(/\d+/g, (match) => {
        return new Intl.NumberFormat("en-US", { minimumFractionDigits: 2 }).format(match);
    });
    ...
    return (
        <div>
            ...
            <div className="d-flex justify-content-center align-items-center" onClick={handleClickOutside}>
                <Button
                    ref={buttonRef} // Ref dem Button-Element zuweisen
                    variant="danger"
                    style={{ fontSize: '100px', padding: '10px 20px', marginBottom: '40px', marginTop: '40px', borderRadius: "30px" }}
                    onClick={sendBuzzer}
                    disabled={buzzerPressedBy}
                >
            ...
            <div className="d-flex justify-content-center align-items-center">
                <InputGroup style={{ width: '900px', height: "150px" }}>
                    <div id="checkBox" onClick={handleCheckBoxClick} style={{ cursor: 'pointer', marginRight: "2vw", border: "2px solid green" }} className="my-auto">
                        <InputGroup.Text htmlFor="readyCheckbox" style={{ cursor: 'pointer' }}>Ready?
                        </InputGroup.Text>
                        <InputGroup.Checkbox id="readyCheckbox" className="mx-auto" />
                    </div>
                    <Form.Control
                        ref={inputRef}
                        as="textarea"
                    ...
                    />
                    <Button onClick={sendMessage} disabled={inputMessage === "" ? true : false}>Nachricht senden</Button>
                </InputGroup>
            </div>
            <div className="d-flex flex-wrap justify-content-center align-items-center" style={{ gap: '20px' }} onClick={handleClickOutside}>
                {allPoints.map((player) => (
                    <div key={userID}>
                        <Card bg="dark" border={!player.isReady ? "success" : 'secondary'} style={{ width: '18rem', marginTop: "30px", borderRadius: "20px" }}>
